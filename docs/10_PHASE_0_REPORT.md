...
| REQ-20260906-003 | F-002, F-021 | T-E2E-002, `docs/06_TEST_ACCEPTANCE.md` §4-D, T-INT-021 | 2, 7 |
...
1. 사용자가 기존 100개 용어 원본을 찾지 못했다고 확인했다(REQ-20260906-003). F-002의 기존 용어 검색 요구는 유지하며, 임의의 신규 목록을 과거 원본으로 표시하지 않는다. Phase 1은 이 결정 없이 진행 가능하다. 다만 Phase 2 진입 전에는 원본 제공·승인된 출처 확인·별도 승인 신규 초기 데이터셋 중 데이터 경로 하나를 결정해야 하며, 그 전에는 F-002의 기존 100개 용어 검색 수용 테스트를 통과 또는 Phase 2 완료로 기록할 수 없다.
